...
const newFirstNames = runners.map(function(item) {
...
const totalTicketPrice = runners.reduce(function(accumulator, item) {
...
// Problem 1
// Get last name and id number for the back of the runners shirts
let lastNameAndID = [];
runners.forEach(function(items) {
  return lastNameAndID.push(`${items.last_name} ${items.id}`);
});
console.log(lastNameAndID);

// Problem 2
// Find runners that dotated more than $200
const donateAbove200 = runners.filter(function(item) {
  return item.donation > 200;
});
console.log(donateAbove200);

// Problem 3
// Find out how many small shirts we need
let runnersSmallSizeShirt = [];
const smallShirts = runners.filter(function(item) {
